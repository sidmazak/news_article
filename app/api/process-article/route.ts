...
// System prompts for each step - Using detailed prompts from test.js
...
// The API key is picked up automatically from the environment variable OPEN_AI_KEY
// https://sdk.vercel.ai/docs/getting-started/setup
// const openaiClient = openai({
//   apiKey: process.env.OPEN_AI_KEY,
// });

// Helper function to process each step
...
export async function POST(request: Request) {
  try {
    const { articleUrl, additionalText } = await request.json();

    console.log('Received request:', { articleUrl, additionalText });

    if (!articleUrl) {
      return NextResponse.json({ error: 'URL is required' }, { status: 400 });
    }

    // Process steps sequentially and collect results
    console.log('Starting processing steps...');

    // Step 1: Extract key information
    console.log('Starting EXTRACTION step...');
    const extractionInput = `${articleUrl}\n${additionalText}`;
    console.log('EXTRACTION Input:', extractionInput);
    const keyInfo = await processStep(EXTRACTION_PROMPT, extractionInput);
    console.log('EXTRACTION Raw AI Output:', keyInfo);

    // Step 2: Extract proper nouns
    console.log('Starting NOUNS step...');
    console.log('NOUNS Input:', articleUrl);
    const nouns = await processStep(NOUN_EXTRACTION_PROMPT, articleUrl);
    console.log('NOUNS Raw AI Output:', nouns);

    // Step 3: Rephrase article
    console.log('Starting REPHRASING step...');
    const rephraseInput = `ARTICLE: ${articleUrl}\nExtracted Details: ${keyInfo}`;
    console.log('REPHRASING Input:', rephraseInput);
    const rephraseArticle = await processStep(REPHRASING_PROMPT, rephraseInput);
    console.log('REPHRASING Raw AI Output:', rephraseArticle);

    // Step 4: Score article
    ...
    console.log('SCORING Raw AI Output:', scoringResult);

    // Step 5: Generate SEO metadata
    console.log('Starting SEO step...');
    const seoInput = `ARTICLE: ${articleUrl}\nExtracted Details: ${keyInfo}\nRephrased Article: ${rephraseArticle}`;
    console.log('SEO Input:', seoInput);
    const seoMetadata = await processStep(SEO_PROMPT, seoInput);
    console.log('SEO Raw AI Output:', seoMetadata);

    // Step 6: Cross-check
    console.log('Starting CROSSCHECK step...');
    const crossCheckInput = `
        ...
        `;
    console.log('CROSSCHECK Input:', crossCheckInput);
    const crossCheckResult = await processStep(CROSS_CHECK_PROMPT, crossCheckInput);
    console.log('CROSSCHECK Raw AI Output:', crossCheckResult);
    ...
    const finalResult = {
      ...
      cross_checked_result: crossCheckResult,
    };

    console.log('Processing complete. Returning JSON response.');
